...
<div class="container">
    <center><h1 style="margin-top: 42px;">Offer</h1></center>
    <center><form method="post" action="{{URL::to('addoffer')}}" enctype="multipart/form-data">
        @csrf
 ...
 <table class="table table-bordered">
    ...
    <tbody>
        ...
        $i=1;
        @endphp
        @foreach($offer as $top)
        <tr>
            <td>{{$i++}}</td>
            <td>{{$top->title}}</td>
            <td>{{$top->description}}</td>
            <td><img src="{{url('/images/'.$top->image)}}" style="width:100px;height:100px; border-radius:10px;" alt=""></td>
            <td><a class="btn btn-success" href="{{URL::to('editoffer',$top->id)}}">edit</a>
            <a class="btn btn-danger" href="{{URL::to('deleteoffer',$top->id)}}">delete</a></td>            
        </tr>
        @endforeach
    </tbody>
 </table>
